...
namespace Infrastructure.FaultHandling
{
	internal class AsyncExecution<TResult>
	{
		...
		private readonly Func<Exception, bool> isFaultException;
		...
		public AsyncExecution(
			...
			Func<Exception, bool> isFaultException,
			...
			CancellationToken cancellationToken)
		{
			...
			this.isFaultException = isFaultException;
		...
		private Task<TResult> ExecuteAsyncImpl(Task ignore)
		{
			...
			if (task == null)
			{
				throw new ArgumentException("task is null", "taskFunc");
			}
			...
			{
				throw new ArgumentException("task is not scheduled.", "taskFunc");
			}
...
#pragma warning restore 0618
			...
			if (!(this.isFaultException(lastError) && this.shouldRetry(this.retryCount++, lastError, out delay)))
			{
	...
	internal class AsyncExecution : AsyncExecution<bool>
	{
		...
		public AsyncExecution(
			...
			bool fastFirstRetry,
			CancellationToken cancellationToken)
			: base(() => StartAsGenericTask(taskAction), shouldRetry, isFaultException, onRetrying, fastFirstRetry, cancellationToken)
		{
		}
